...
func (cn *conn) Query(query string, args []driver.Value) (driver.Rows, error) {
	if cn.isProcedure(args) {
		return cn.QueryProcedure(query, args)
	}
...
func (cn *conn) Exec(query string, args []driver.Value) (res driver.Result, err error) {
	...
	if cn.isProcedure(args) {
		res, err = cn.ExecProcedure(query, args)
		return res, err
	}
...
func (cn *conn) isProcedure(args []driver.Value) bool {
...
func (cn *conn) QueryProcedure(query string, args []driver.Value) (res driver.Rows, err error) {
	var newArgs []driver.Value
	var outArgs []driver.Value
	var outList []sql.Out
	var posList []int
	for n, v := range args {
		switch d := v.(type) {
		case sql.Out:
			outList = append(outList, sql.Out(d))
			posList = append(posList, n)
			switch sql.Out(d).Dest.(type) {
			case *uint8, *uint16, *uint32, *int16, *int, *int64, *uint64:
				outArgs = append(outArgs, int64(0))
			case *float32, *float64:
				outArgs = append(outArgs, float64(0))
			case *string:
				value := sql.Out(d).Dest.(*string)
	...
	newArgs = append(newArgs, outArgs...)

	rows, err := cn.Query(query, newArgs)
	if err != nil {
		return rows, err
	}

	var outDrivers []driver.Value
	for _, out := range outList {
		outDrivers = append(outDrivers, out.Dest)
	}

	err = rows.Next(outDrivers)
	if err != nil {
		return rows, err
	}
	err = assignArgsFromOutDrivers(args, outDrivers, posList)
	if err != nil {
		return rows, err
	}
...
func (cn *conn) ExecProcedure(query string, args []driver.Value) (res driver.Result, err error) {
	var newArgs []driver.Value
	var outArgs []driver.Value
	var outList []sql.Out
	var posList []int
		...
		case sql.Out:
			...
			case *string:
				...
				if *value != "" {
					...
					outArgs = append(outArgs, sql.Out(d).Dest)
				}
			default:
				outArgs = append(outArgs, sql.Out(d).Dest)
			}
...
func assignArgsFromOutDrivers(args []driver.Value, outDrivers []driver.Value, posList []int) error {
	for index, value := range outDrivers {
		switch args[posList[index]].(type) {
		case sql.Out:
			switch args[posList[index]].(sql.Out).Dest.(type) {
			case *int:
				*args[posList[index]].(sql.Out).Dest.(*int) = value.(int)
			case *string:
				strValue, err := getStrValue(value)
				if err != nil {
			...
			case *float32:
				*args[posList[index]].(sql.Out).Dest.(*float32) = value.(float32)
			case *float64:
				*args[posList[index]].(sql.Out).Dest.(*float64) = value.(float64)
			case *uint:
				*args[posList[index]].(sql.Out).Dest.(*uint) = value.(uint)
			case *uint8:
				*args[posList[index]].(sql.Out).Dest.(*uint8) = value.(uint8)
			case *uint16:
				*args[posList[index]].(sql.Out).Dest.(*uint16) = value.(uint16)
			case *uint32:
				*args[posList[index]].(sql.Out).Dest.(*uint32) = value.(uint32)
			case *uint64:
				*args[posList[index]].(sql.Out).Dest.(*uint64) = value.(uint64)
			default:
				return errors.New("drivers is not support sql.Out Dest type")
			}
		}
	}
...
func getStrValue(value driver.Value) (string, error) {
	...
	case uint8, uint16, uint32, int16, int, int64, uint64:
		uv := value.(int)
		result = strconv.Itoa(uv)
	case float32, float64:
		uv := value.(float64)
		iuv := int64(uv)
		if uv-float64(iuv) == 0 {
			result = strconv.FormatFloat(uv, 'f', 0, 64)
		} else {
			result = strconv.FormatFloat(uv, 'f', 2, 64)
		}
...
func (st *stmt) isProcedure(args []driver.Value) bool {
	var isProcedure bool
	for _, v := range args {
		switch v.(type) {
		case sql.Out:
...
func (st *stmt) ExecProcedure(args []driver.Value) (res driver.Result, err error) {
	var newArgs []driver.Value
	var outArgs []driver.Value
	var outList []sql.Out
	var posList []int
